    ...
    user.findOne(params, (err, doc) => {
        if (err) {
            ...
            if (doc) {
              ...
              const uId = platForm + r1 + createDateStr + r2
              let u = new user({
                userName: userName,
                userPwd: userPwd,
                tel: tel,
...
router.get('/cart', (req, res, next) => {
    ...
    user.findOne({uId: userId}, (err, doc) => {
        if (err) {
            res.json({
                status: -1,
                msg: err.message
...
router.post('/cartCheckAll', (req, res, next) => {
    ...
    console.log(userId)
    user.findOne({uId: userId}, (err1, doc1) => {
        if (err1) {
            res.json({
                status: -1,
                msg: err1.message
  ...
  user.findOne({uId:userId}, (err1, doc1) => {
      if (err1) {
        ...
        if (doc1) {
          ...
          const createDate = new Date().Format('yyyy-MM-dd hh:mm:ss')
          const createDateStr = new Date().Format('yyyyMMddhhmmss')
          const addressId = platForm + r1 + createDateStr + r2
          params.addressId = addressId
          doc1.addressList.push(params)
          doc1.save((err2, doc2) => {
            if (err2) {
              res.json({
                status: -1,
